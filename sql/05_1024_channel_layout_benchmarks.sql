...
-- ---------------------------------------------------------------------------
-- D. Wide table layout: one physical column per channel.
...
-- D1. Create and load sensor_payloads_wide (... ch0001 float8, ...).
DO $$
DECLARE
    channel_defs text;
    channel_names text;
...
-- D2. Generate one-scan min/max SQL for every wide-table channel.
...
-- D3. One-channel min/max example.
EXPLAIN (ANALYZE, BUFFERS, TIMING)
